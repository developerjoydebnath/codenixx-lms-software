...
import { IconBan, IconCircleCheck, IconCreditCardPay, IconEdit, IconLoader2, IconUser } from "@tabler/icons-react";
import Link from "next/link";
import { useState } from "react";
import { Button } from "../ui/button";

export default function BroadbandUserProfileHead({
    imagePath = "",
    name = "John Doe",
    ...
    const handleEnableDisable = () => {
        ...
        setTimeout(() => {
            show_toast("success", "User Disabled Successfully");
        ...
        <div className={cn("flex items-center justify-between", className)}>
            ...
            <div className="flex flex-wrap gap-2 sm:flex-nowrap">
                {checkAccessPermission({
                    ...
                    <Link href={`/broadband-users/${uid}/update`}>
                ...
                {checkAccessPermission({
                    ...
                    <Link href={`/broadband-users/${uid}/payment`}>
                ...
                {checkAccessPermission({
                    ...
                    true ? (
                        <Button
                            ...
                            className="gap-1.5 bg-destructive/20 px-2.5 text-destructive hover:bg-destructive hover:text-base-0 active:bg-destructive/80 active:text-base-0 xmd:w-[105px] xmd:px-4"
                        >
                            {enableDisableLoading ? (
                                <IconLoader2 className="animate-spin-slow" size={20} />
                            ) : (
                                <>
                                    <IconBan size={20} />
                                    <span className="hidden xmd:block">Disable</span>
                                </>
                            )}
                        </Button>
                    ) : (
                        <Button
                            disabled={enableDisableLoading}
                            onClick={handleEnableDisable}
                            className="gap-1.5 bg-green-500/20 px-2.5 text-green-500 hover:bg-green-500 hover:text-base-0 active:bg-green-500/80 active:text-base-0 xmd:w-[105px] xmd:px-4"
                        >
                            {enableDisableLoading ? (
                                <IconLoader2 className="animate-spin-slow" size={20} />
                            ) : (
                                <>
                                    ...
                                    <span className="hidden xmd:block">Enable</span>
                                </>
                            )}
                        </Button>
                    )
